...
try {
    ...
    $sth->execute([$usuario]);
    $u = $sth->fetch();
    if (!$u || $u['status'] !== 'ativo') { http_response_code(401); echo json_encode(['status'=>'error','message'=>'Usuário inválido']); exit; }
    if (!empty($u['escola_id']) && ($u['escola_status'] ?? 'ativo') !== 'ativo') { http_response_code(403); echo json_encode(['status'=>'error','message'=>'Escola inativa']); exit; }
    if (!password_verify($senha, $u['senha_hash'])) { http_response_code(401); echo json_encode(['status'=>'error','message'=>'Senha inválida']); exit; }
